...
def test_delisted_symbol_visible_before_delisting(tmp_path):
    ...
    u.add("LEH", datetime(2000, 1, 1), datetime(2008, 9, 15), knowledge_time=datetime(2000, 1, 1))
    # A survivor that never left.
    u.add("AAPL", datetime(2000, 1, 1), None, knowledge_time=datetime(2000, 1, 1))
...
def test_membership_respects_knowledge_time(tmp_path):
    ...
    u.add("LEH", datetime(2000, 1, 1), included_to=None, knowledge_time=datetime(2000, 1, 1))
    # Then corrected — same start date, with end date this time.
    u.add(
        "LEH",
        datetime(2000, 1, 1),
        included_to=datetime(2008, 9, 15),
