...
def rsi_v1(df: pl.DataFrame) -> pl.DataFrame:
    ...
    out = out.with_columns(
        (100.0 - 100.0 / (1.0 + pl.col("_ag") / pl.col("_al"))).alias("rsi14")
    )
...
def rsi_v2(df: pl.DataFrame) -> pl.DataFrame:
    ...
    out = out.with_columns(
        pl.col("_g").ewm_mean(alpha=alpha, adjust=False, min_samples=period).over("symbol").alias("_ag"),
        pl.col("_l").ewm_mean(alpha=alpha, adjust=False, min_samples=period).over("symbol").alias("_al"),
    )
    out = out.with_columns(
        (100.0 - 100.0 / (1.0 + pl.col("_ag") / pl.col("_al"))).alias("rsi14")
    )
...
def bollinger_v1(df: pl.DataFrame) -> pl.DataFrame:
    ...
    out = out.with_columns(
        ((pl.col("close") - pl.col("bb_lower"))
         / (pl.col("bb_upper") - pl.col("bb_lower"))).alias("bb_pctb")
    )
